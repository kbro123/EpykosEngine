...
Definitions are data (D36, D42): `blueprints/conventions/*.json` is the conventions registry, located at build time
(`EPYKOS_BLUEPRINTS_DIR` = the source tree's `blueprints/`) or by `$EPYKOS_BLUEPRINTS`; the citations are in `docs/G4_BUNDLE.md`.
