...
- `adjoint_m1_adjoint.json` + `m2.md`: the M2 gate round (M2/m2-gate) — value + adjoint on the M1 book
  (`adjoint_m1_adjoint_bench` with
  `--benchmark_filter='^BM_AdjointEval/|^BM_AdjointEvalBatch/tile:(128|256)/lane_tile:8$|^BM_AdjointBuild$'`: B=1
  at the D15 tiles, B=64 at lane tile 8, the constructor), informational: 7.34x the interpreter's value-only B=1
  median. The six rows are in `baseline.json` under the run name `adjoint_m1_adjoint`, the name `run.sh` derives
  from the binary, so a plain `bench/run.sh build/release/bench/adjoint_m1_adjoint_bench` is gated at 1.25x on those
  six rows (an unfiltered sweep's other rows are reported as new, not gated). Measured and seeded as run `m2`
  (`m2.json`, perf commit a72f525) by M2/m2-gate; re-keyed by M2/m2-fix with the medians unchanged, because a
  baseline under an ad hoc `--name` gated only a run that repeated `--name m2` and the default invocation was
  refused (D34).
