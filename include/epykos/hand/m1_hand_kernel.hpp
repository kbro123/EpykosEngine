...
//     the linmap, stored as (knot0, knot1, w0, w1) rows (R7: two non-zeros per row);
...
// the coupon pass runs on lane tiles of 8 so the leg accumulators stay in registers, and every
// inner loop is plain C++ the compiler auto-vectorises (no intrinsics).
//
...
class M1HandKernel {
 public:
  ...
  int lane_tile() const noexcept;                            // the batch lane tile (8)
  const std::vector<double>& times() const noexcept { return time_; }
 ...
 private:
  ...
  void coupon_pass(int stride, int B, double* swap_pv) const;
  ...
  // Plain rows of swap i: [plain_begin[i], fixed_end[i]) are its fixed leg in period order,
  // [fixed_end[i], plain_begin[i+1]) its realised first float coupon (0 or 1 row).
  int n_plain_ = 0;
  std::vector<int> plain_begin_, fixed_end_;
  std::vector<int> plain_time_;      // index of e_j in the time table
  std::vector<double> plain_coef_;   // N·τ·K or N·τ·R

  // Float rows of swap i: [float_begin[i], float_begin[i+1]) in period order.
  int n_float_ = 0;
