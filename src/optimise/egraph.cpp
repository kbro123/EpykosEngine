...
// ---- the application memo (D62) -----------------------------------------------------------------

bool EGraph::sync_memo(const std::vector<const rewrite::Rule*>& rules) {
  bool same = memo_rules_.size() == rules.size();
  if (same) {
    for (std::size_t i = 0; i < rules.size(); ++i) {
      if (memo_rules_[i] != rules[i]) { same = false; break; }
      if (rules[i] != nullptr && memo_rule_names_[i] != rules[i]->name()) { same = false; break; }
    }
  }
  if (!same) {
    memo_.clear();
    // A slot index taken from a different rules vector means nothing; reopen every plan node to
    // every rule rather than silently restricting the search against stale indices.
    for (std::vector<PlanNode>& tier : plans_) {
      for (PlanNode& n : tier) n.max_rule_slot = -1;
    }
    memo_rules_ = rules;
    memo_rule_names_.assign(rules.size(), std::string{});
    for (std::size_t i = 0; i < rules.size(); ++i) {
      if (rules[i] != nullptr) memo_rule_names_[i] = rules[i]->name();
    }
    rule_kind_.assign(rules.size(), ProposalKind::Unknown);
  }
  // Grow to the current node count; a node added since the last call starts with an empty memo.
  memo_.resize(programs_.size());
  for (std::vector<SiteMemo>& row : memo_) row.resize(rules.size());
  return same;
}

void EGraph::note_rule_kind(std::size_t slot, ProposalKind kind) {
  if (slot >= rule_kind_.size()) return;
  ProposalKind& cur = rule_kind_[slot];
  if (cur == ProposalKind::Unknown) cur = kind;
  else if (cur != kind) cur = ProposalKind::Mixed;
}
...
SaturationReport EGraph::saturate(const std::vector<const rewrite::Rule*>& rules, const SaturationLimits& limits) {
  ...
  // The memo and the rule-kind table are keyed by slot in THIS `rules` vector; sync_memo discards
  // both if the caller has changed the rule set since the last call. `rule_kind_` is needed by the
  // re-firing policy even when the memo itself is off, so this runs either way.
  sync_memo(rules);
  ...
  for (int iter = 0; iter < limits.max_iterations; ++iter) {
    ...
    if (!limits.dedup_before_construction) {
      // Wiped at the top of EVERY round, so the loop below re-matches and re-proposes exactly as
      // D49's original did — the "before" behaviour, reproducible on demand.
      for (std::vector<SiteMemo>& row : memo_) {
        for (SiteMemo& m : row) m = SiteMemo{};
      }
    }
    ...
    // D62, and D49 point 3's own flagged "Known limitation" in its cheapest sound form: only a
    // program CLASS's representative (the lowest-id member, exactly the node `extract` visits) is
    // matched and grows a plan tier. A non-representative node is BY DEFINITION congruent to its
    // representative — same `ir::Program` content, same empty plan-tier root — so by rule.hpp's
    // purity contract every site it would match and every proposal it would make are identical to
    // the representative's. Skipping it costs no reachable content and saves both the match/
    // propose work and, far more importantly, a whole duplicate plan tier per duplicate node
    // (measured on the 60-trade Stage A fixture: 71 program nodes, 22 classes — 69% of all plan-
    // tier growth was on nodes `extract` never looks at). One honest interaction with
    // RefirePolicy::NoFreshCrossRule, stated rather than buried: the self-chaining privilege
    // follows the REPRESENTATIVE's own producing rule, so when two different rules reach the same
    // content it is the lower-id derivation's rule that may continue the chain.
    std::vector<std::uint8_t> is_representative(static_cast<std::size_t>(program_frontier),
                                                limits.dedup_before_construction ? 0 : 1);
    if (limits.dedup_before_construction) {
      for (int rep : program_class_representatives()) {
        if (rep < program_frontier) is_representative[static_cast<std::size_t>(rep)] = 1;
      }
    }
    ...
    for (std::size_t slot = 0; slot < rules.size(); ++slot) {
      const rewrite::Rule* rule = rules[slot];
      if (rule == nullptr) continue;
      std::size_t sites_matched = 0;
      std::size_t nodes_added = 0;
      std::size_t proposals_memoised = 0;
      std::size_t nodes_blocked = 0;
      bool cut = false;
      std::string note;

      for (int pid = 0; pid < program_frontier && !cut; ++pid) {
        if (!is_representative[static_cast<std::size_t>(pid)]) continue;  // see above
        // (B) D62's re-firing policy. A rule this graph has only ever seen propose STRUCTURALLY is
        // not matched against a program node some OTHER structural rule produced. The root
        // (parent < 0) is always matched against everything; a rule whose kind is still Unknown
        // (it has proposed nothing yet) is never restricted. See egraph.hpp's RefirePolicy comment
        // for exactly which candidates this makes unreachable.
        // The mutant makes the policy refuse a rule its OWN output too, collapsing "homogeneous
        // structural chains" to "at most one structural rewrite ever" — which would silently drop
        // the r5.group_formation-applied-twice candidate (D54 experiment 2).
        const bool rule_is_this_nodes_own =
            !epykos::mutant("eg.refire_blocks_self_chain") &&
            programs_[static_cast<std::size_t>(pid)].rule == rule->name();
        if (limits.refire != RefirePolicy::AllRules &&  // NoFreshCrossRule and everything above it
            programs_[static_cast<std::size_t>(pid)].parent >= 0 &&
            rule_kind_[slot] == ProposalKind::Structural && !rule_is_this_nodes_own) {
          ++nodes_blocked;
          continue;
        }
        ...
        if (!memo.matched) {
          memo.sites = rule->match(prog, ctx);
          // The mutant retires a whole (node, rule) pair on its FIRST site, so every later site
          // this rule matches on this node is never applied at all — a dedup that has quietly
          // become a narrowing of the search.
          if (limits.dedup_before_construction && epykos::mutant("eg.memo_ignores_site") && memo.sites.size() > 1) {
            memo.sites.resize(1);
          }
          memo.kind.assign(memo.sites.size(), static_cast<std::uint8_t>(ProposalKind::Unknown));
          memo.plan_parents_done.assign(memo.sites.size(), 0);
          memo.matched = true;
          sites_matched += memo.sites.size();
        }

        for (std::size_t si = 0; si < memo.sites.size(); ++si) {
          const rewrite::MatchSite& site = memo.sites[si];
          const int frontier = plan_frontier[static_cast<std::size_t>(pid)];

          // (A) The memo's two skips. A structural site has exactly one answer and it has already
          // been given (queued, deduped against an existing node, or rejected as invalid) — never
          // pay for the Program clone, the ir::validate and the ir::serialize again. An annotation
          // site's own delta is likewise fixed; only its node's plan tier grows, so the only work
          // left is merging onto plan parents not yet seen.
          if (memo.kind[si] == static_cast<std::uint8_t>(ProposalKind::Structural)) {
            ++proposals_memoised;
            continue;
          }
          if (memo.kind[si] == static_cast<std::uint8_t>(ProposalKind::Annotation) &&
              memo.plan_parents_done[si] >= frontier) {
            ++proposals_memoised;
            continue;
          }
          ...
          if (proposal.is_structural()) {
            if (program_total + pending_programs_.size() >= limits.max_program_nodes) {
              ...
              break;  // NOT memoised: this site was never actually applied.
            }
            memo.kind[si] = static_cast<std::uint8_t>(ProposalKind::Structural);
            note_rule_kind(slot, ProposalKind::Structural);
            try {
              ...
              // It is memoised all the same: `propose` is a pure function, so the next round's
              // identical call would throw the identical exception.
            ...
            memo.kind[si] = static_cast<std::uint8_t>(ProposalKind::Annotation);
            note_rule_kind(slot, ProposalKind::Annotation);
            // Plan parents below `plan_parents_done[si]` were merged in an earlier round: the
            // delta and the parent are both unchanged, so the merge result is unchanged, so it is
            // either already a node of this tier or was already rejected by `plan_known` — either
            // way re-doing it can add nothing.
            int plid = memo.plan_parents_done[si];
            for (; plid < frontier; ++plid) {
              const PlanNode& base = plans_[static_cast<std::size_t>(pid)][static_cast<std::size_t>(plid)];
              // (B) in the plan tier: PipelineOrderedPlans lets a rule extend only plan nodes
              // every one of whose own rules sits at a strictly lower slot, so a plan derivation
              // runs in the caller's declared rule order and uses each rule at most once. See
              // egraph.hpp's RefirePolicy comment for the combinations that costs.
              if (limits.refire == RefirePolicy::PipelineOrderedPlans &&
                  base.max_rule_slot >= static_cast<int>(slot)) {
                continue;
              }
              ...
              pending_plans_.push_back(
                  PendingPlan{pid, plid, std::move(merged), rule->name(), ex, static_cast<int>(slot)});
              ++nodes_added;
              any_queued = true;
            }
            // Exactly the parents actually visited, so a bound that cut this loop short leaves
            // the rest to be retried rather than silently skipped.
            memo.plan_parents_done[si] = plid;
            if (cut) break;
          } else {
      ...
      report.log.push_back(SaturationLogEntry{iter, rule->name(), sites_matched, nodes_added,
                                              proposals_memoised, nodes_blocked, cut, note});
    }

    rebuild();
    // rebuild() appended this round's nodes; give each one a memo row before the next round.
    memo_.resize(programs_.size());
    for (std::vector<SiteMemo>& row : memo_) row.resize(rules.size());
...
void EGraph::rebuild() {
  ...
  for (PendingPlan& pending : pending_plans_) {
    ...
    node.history.push_back(pending.rule);
    node.max_rule_slot = pending.rule_slot > parent.max_rule_slot ? pending.rule_slot : parent.max_rule_slot;
