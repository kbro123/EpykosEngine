...
TEST(M1Book, Statistics) {
  ...
  EXPECT_EQ(b.knot_t, fixtures::knot_times);
  EXPECT_EQ(b.z0, fixtures::record_state);
}
...
TEST(M1Book, RowTableIsConsistentWithTheCalendar) {
  ...
  for (int i = 0; i < b.n_swaps; ++i) {
    ...
    EXPECT_EQ(fixtures::fixed_row_begin(b, i), b.row_begin[s]);
    EXPECT_EQ(fixtures::float_row_begin(b, i), b.row_begin[s] + T);
    for (int leg = 0; leg < 2; ++leg) {
      for (int j = 1; j <= T; ++j) {
        ...
        const bool realised = leg == fixtures::float_leg && j == 1 && b.seasoned[s];
...
TEST(M1Book, DeterministicBitwise) {
  const fixtures::Book a = fixtures::make_m1_book(20260922);
  const fixtures::Book b = fixtures::make_m1_book(20260922);
  ...
  const fixtures::Book c = fixtures::make_m1_book(1);
...
TEST(M1Book, ParRatesReproduceKOverOnePlusEps) {
  ...
  for (int i = 0; i < b.n_swaps; ++i) {
    const auto s = static_cast<std::size_t>(i);
    const double par = fixtures::par_rate(b, i, b.z0.data());
...
TEST(M1Book, RealisedRateIsZ1TimesOnePlusU) {
  const fixtures::Book b = fixtures::make_m1_book();
  const double z1 = epykos::curve::linear::zero_rate(b.knot_t.data(), b.z0.data(), fixtures::n_knots, 1.0);
  EXPECT_EQ(z1, 0.0420);
  for (int i = 0; i < 200; ++i) {
...
TEST(M1Batch, LayoutAndRecordPoint) {
  const fixtures::Batch bt = fixtures::make_m1_batch();
  ...
  bt.state(0, z);
  for (int k = 0; k < 12; ++k) {
    EXPECT_EQ(z[k], fixtures::record_state[static_cast<std::size_t>(k)]);
    EXPECT_EQ(bt.at(k, 0), fixtures::record_state[static_cast<std::size_t>(k)]);
    EXPECT_EQ(bt.z[static_cast<std::size_t>(k * 64)], fixtures::record_state[static_cast<std::size_t>(k)]);
  }
  double s1 = 0.0;
  double s2 = 0.0;
  int n = 0;
  for (int b = 1; b < 64; ++b) {
    ...
    bool any_diff = false;
    for (int k = 0; k < 12; ++k) {
      const double delta = z[k] - fixtures::record_state[static_cast<std::size_t>(k)];
  ...
  // Deterministic.
  const fixtures::Batch again = fixtures::make_m1_batch();
...
TEST(M1Book, DumpCsvWritesBothTables) {
  ...
  in.close();
  EXPECT_EQ(blank, 2);
  EXPECT_EQ(lines, (1 + b.n_swaps) + 1 + (1 + b.n_rows) + 1 + (1 + fixtures::n_knots));
